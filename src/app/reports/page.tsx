...
export default function ReportsPage() {
  return (
    <div className="space-y-5">
      <PageHeader title="التقارير" description="تقارير تشغيلية قابلة للتصدير Excel والطباعة PDF من المتصفح." />
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {REPORTS.map((report) => (
          <Link className="card group p-5 transition hover:-translate-y-0.5 hover:border-[#0f766e]" href={`/reports/${report.slug}`} key={report.slug}>
            ...
            <span className="mt-4 inline-block text-sm font-semibold text-[#0f766e]">فتح التقرير</span>
          </Link>
        ))}
      </div>
    </div>
